...
def show_go_screen(score, clientsocket):
    screen.fill(BLACK)
    draw_text(screen, "True color", 64, width/2, height/4)
    draw_text(screen, "Arrows to move, Space to fire",24,width/2,height/2)
    draw_text(screen, "Wait for server to start game",24,width/2,height*3/4)
    high_score(score)
    pygame.display.flip()
    waiting = True
    while waiting:
        #clock.tick(FPS)
        s_status = clientsocket.recv(1024)
        if(s_status==b'T'):
            waiting = False
...
def oscore_card(score1, score2, clientsocket):
    screen.fill(BLACK)
    if(score1>score2):
    ...
    draw_text(screen, "Wait for server to start game",24,width/2,height*4/5)
    pygame.display.flip()
    waiting = True
    while waiting:
        s_status = clientsocket.recv(1024)
...
def yscore_card(score, clientsocket):
    ...
    clientsocket.sendall(bytes([score]))
    pygame.display.flip()
    waiting = True
    while waiting:
        s_score = clientsocket.recv(1024)
        o_score = int.from_bytes(s_score, "little")
        oscore_card(score, o_score, clientsocket)
        waiting = False


def wait_screen(clientsocket):
    screen.fill(BLACK)
    draw_text(screen, "Wait for opponent to select charcter", 40, width/2, height/4)
...
def ship_selection(clientsocket):
    screen.fill(BLACK)
...
def run_client():
    ...
    while running:
        if game_over:
            if(count>0):
                show_go_screen(score, clientsocket)
                choice = ship_selection(clientsocket)
                ship_selection(clientsocket)
                wait_screen(clientsocket)
        ...
        if hits:
            game_over = True
            sleep(0.5)
            yscore_card(score, clientsocket)
        ...
        #draw_text(screen , "Opponent's score:"+str(s_score), 22, 3*width/4, 10)
